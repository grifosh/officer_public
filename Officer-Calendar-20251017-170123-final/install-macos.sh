...
setup_environment() {
    ...
    if [ -f ".env" ]; then
        ...
        echo
        if [[ ! $REPLY =~ ^[Yy]$ ]]; then
            print_info "Используем существующий файл .env"
            return 0
        fi
    fi
    ...
    read -p "Пароль БД [$DB_PASSWORD]: " input_db_password
    DB_PASSWORD=${input_db_password:-$DB_PASSWORD}
    
    read -p "Хост БД [localhost]: " input_db_host
    DB_HOST=${input_db_host:-localhost}
    
    read -p "Порт БД [5432]: " input_db_port
    DB_PORT=${input_db_port:-5432}
    
    read -p "Порт сервера [$SERVER_PORT]: " input_server_port
    SERVER_PORT=${input_server_port:-$SERVER_PORT}
    
    read -p "Режим (development/production) [development]: " input_environment
